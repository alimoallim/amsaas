<template>
  <WorklistLayout
    ...
    :count="meta.total"
    description="Review charges generated from approved meter readings before invoice consolidation."
  >
    <template #actions>
      <ErpButton variant="ghost" size="sm" :to="{ name: 'ChargeApproval' }">
        Approval guide
      </ErpButton>
      <ErpButton
        variant="secondary"
    ...
    <template #kpis>
      ...
      <KpiStrip class="grid-cols-2 lg:grid-cols-4">
        ...
        <KpiCard label="Pending (page)" :value="summary.pending" />
        <KpiCard label="Cancelled (page)" :value="summary.cancelled" />
      </KpiStrip>
    </template>
    ...
    <template #table>
      <DataTable
        ...
        empty-description="Approve meter readings to generate utility charges, or change filters."
  ...
  <ErpModal
    ...
    title="Reject charge"
    subtitle="Rejection requires a reason. Cancelled charges will not be invoiced."
