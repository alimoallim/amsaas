  ...
  <div class="space-y-6 max-w-5xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-slate-900">Create Charge Type</h1>
          <p class="mt-1 text-sm text-slate-500">
            Configure baseline parameters, financial rules, pricing metrics, and general ledger tracking routes.
          </p>
        </div>
        <RouterLink
          :to="{ name: 'ChargeTypes' }"
    ...
    <div v-if="serverError" class="rounded-lg border border-red-200 bg-red-50 p-4 text-sm font-medium text-red-700 shadow-sm flex items-start">
    ...
    <form @submit.prevent="submit" class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden space-y-8">
      ...
      <div class="p-6 border-b border-slate-100">
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
          <div>
            ...
            <input id="name" type="text" v-model="form.name" @input="autoGenerateCode" placeholder="e.g., Residential Rent" class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-slate-500 focus:outline-none" :class="{ 'border-red-300': errors.name }" />
          ...
          <div>
            ...
            <input id="code" type="text" v-model="form.code" placeholder="e.g., RENT_RESIDENTIAL" class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm uppercase font-mono bg-slate-50 focus:bg-white focus:border-slate-500 focus:outline-none" :class="{ 'border-red-300': errors.code }" />
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-5">
          ...
          <div class="md:col-span-2">
            ...
            <div class="relative mt-1 rounded-md shadow-sm">
              <input id="default_amount" type="number" step="0.01" v-model.number="form.default_amount" placeholder="0.00" class="block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-slate-500 focus:outline-none" />
            </div>
          </div>
      ...
      <div class="p-6 bg-slate-50/50 border-b border-slate-100">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div class="flex items-start p-4 rounded-lg border border-slate-200 bg-white shadow-sm">
            ...
            <label id="affects_occupancy" class="ml-3 text-sm"><span class="block font-medium text-slate-700">Affects Lease Occupancy Metrics</span><span class="text-xs text-slate-400">Factors execution rules into occupancy models.</span></label>
          </div>
      ...
      <div class="bg-slate-50 px-6 py-4 flex items-center justify-end space-x-3 border-t border-slate-200">
        ...
        <button type="submit" :disabled="submitting" class="inline-flex justify-center rounded-lg border border-transparent bg-slate-900 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-slate-800 focus:outline-none disabled:opacity-50">
          ...
          {{ submitting ? 'Persisting Configuration Data...' : 'Save Component Model' }}
        </button>
      </div>
    </form>
  </div>
...
// Initial reactive state aligned perfectly with model properties and defaults
const form = reactive({
  name: '',
  short_name: '',
  code: '',
...
async function submit() {
  ...
  try {
    const payload = {
      ...
      // Handle cleaning numeric assignments safely to guard database execution bounds
    ...
    if (error.response?.status === 422) {
      ...
      serverError.value = 'Validation tracking failure. Please fix structural errors highlighted below.'
      return
    }
    serverError.value = error.response?.data?.message || 'A generic internal processing anomaly occurred on the remote container.'
  } finally {
